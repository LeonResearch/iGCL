...
def Fast_Contrastive_Loss(Z,mu,sigma2,tau,num_samples,device):
...
def Reconstruction_Loss(preds, labels, norm, pos_weight):
...
def VGAE_Loss_X(preds, X, mu, logvar, n_nodes):
    #cost = F.mse_loss(preds,X)
    cost = F.cross_entropy(preds,X)
...
def KLD_gaussian(mu_p, std_p, mu_q, std_q):
    """Analytical KLD between 2 Gaussians."""
    ps2 = std_p**2 + 1e-16
    qs2 = std_q**2 + 1e-16
    
    return (ps2/qs2 + ((mu_p-mu_q)**2)/qs2 + torch.log(qs2/ps2) - 1.0).sum()*0.5
